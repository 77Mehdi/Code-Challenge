...
export const Card = React.memo(({ card, index, hovered, setHovered, image }) => (
  ...
  >
    ...
    <img
      src={image}
      alt={`Skip image ${index + 1}`}
      className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
    />

    {/* Overlay */}
    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent p-6 flex flex-col justify-end transition-opacity duration-300 opacity-100">
      ...
      <ul className="text-sm text-gray-200 space-y-1">
        ...
        {!card.allowed_on_road && (
          <li className=" flex items-center gap-1"><strong>Not Allowed On The Road</strong><TiWarning  className=" text-amber-300 text-[19px]"/></li>
        )}
        
        
      </ul>
      <button className="mt-4 self-start bg-white text-black px-4 py-2 rounded hover:bg-gray-200 transition flex items-center  gap-2 ">
        Select Skip  <HiArrowNarrowRight />
      </button>
    </div>
  </div>
));
